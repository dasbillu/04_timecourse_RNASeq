...
dat <- read.csv("./data/activity_data/pogo_C01_activity_tube_21Nov22.csv", 
...
## Only for Day 15 and 16
p2 <- bar %>% 
  # filter to keep only data for day 15 and 16
  filter(day %in% c("15","16")) %>% 
  # make the summary table
  summarySE(.,
            # specify your measurevar
            measurevar= "zmedian", 
  ...
  labs(y = "mean (± SE)",
       x = "hours since lights are on",
       subtitle="Median number of ants",
       caption = "\n Number of ants observed in the tube,\n at each time point")
